...
const digiRows =rows.map(r => (r.split('').filter(char => (whitelist.includes(char))))).forEach(row => (calibrationLog.push(row[0] + row[row.length - 1])));
calibrationLog.forEach(a => (result += parseInt(a)));
