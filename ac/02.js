...
const input =`Game 1: 2 red, 2 green; 6 red, 3 green; 2 red, 1 green, 2 blue; 1 red
...
const colorlist = 'redgreenblue';

let games0 = input.split('\n');


games0 = games0.map((g)=> (g.split(':')[1]))
               .map((p) => (p.split(';'))
               .map((s) => (s.split(','))
               ));
// console.log(games0);            
...
games0.forEach((g, gidx) =>{
    ...
    g.forEach((p, pidx) =>{
        ...
        p.forEach((s, sidx) =>{
            const color = s.split('').filter(char => (colorlist.includes(char)))[0]; 
            const num = parseInt(s.split('').filter(char => (numlist.includes(char))).join('')); 
            colorsums[color] = colorsums[color] + num;  
            // console.log('color:',color, 'Num:', num, `colorsums${[color]}`, colorsums[color]);     
            
            if (gameMaxColor[color] < num) {
                // console.log(color, '=>', num, '(', gameMaxColor[color], ')');
                gameMaxColor[color] = num;
            }
        })

        // console.log((colorsums.r > 12) || (colorsums.g >13) || (colorsums.b > 14));
    ...
    powerSum += gameMaxColor.r * gameMaxColor.g * gameMaxColor.b;
    console.log(gameMaxColor.r, gameMaxColor.g, gameMaxColor.b, (gameMaxColor.r * gameMaxColor.g * gameMaxColor.b) );        
...
console.log('gg: ', goodGames);
console.log('PS', powerSum);



// games[3]['p3']['s3']['b'] = 10;   
//  console.log(games[2]['p3']['s3']['b']);
//  console.log(games0);
// console.log(games);
