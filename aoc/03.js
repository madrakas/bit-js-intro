...
rows.forEach((r, ridx) => {
    const keylocations = [];
    r.split('').forEach((char, charidx)=> {
            if(!blacklist.includes(char)){
                keylocations.push (charidx);
            }
        })
    keylocations.length && keys.push ({[ridx]: keylocations});
});

keys.forEach(k =>{
    // console.log('y', parseInt(Object.keys(k)[0]), 'x-ai', k[Object.keys(k)[0]] );
   
});


// look for numbers touching keys

keys.forEach(k =>{
        //left ='';
        const y = parseInt(Object.keys(k)[0]);
        const xs= k[Object.keys(k)[0]];
        
        // console.log('xs:', xs, 'y:', y);

        xs.forEach(x => {
            if (x > 0){
                if(numbers.includes((rows[y][x - 1]))){              //left
                    // console.log(y, x - 1);
                    numExtractor(y, x - 1);
                 }     
            }
            ...
            if (x < rows[y].length - 1){
                if (numbers.includes((rows[y][x + 1]))){             //right
                    // console.log(y, x + 1);
                    numExtractor(y, x + 1);
                }   
            }
            ...
            if (y > 0){
                if(numbers.includes((rows[y - 1][x]))){             //up
                    // console.log(y - 1, x);
                    numExtractor(y - 1, x);
                }     
            }
            ...
            if (y < rows.length - 1){
                if(numbers.includes((rows[y + 1][x]))){             //down
                    // console.log(y + 1, x);
                    numExtractor(y + 1, x);
                }                               
            }    
            ...
            if ((y > 0) &&  (x > 0)){
                if(numbers.includes((rows[y - 1][x - 1]))){         //leftup
                    // console.log(y -1, x -1);
                    numExtractor(y -1, x -1);
                }     
            }

            if ((y > 0) && (x < rows[y].length - 1)){
                if(numbers.includes((rows[y - 1][x + 1]))){         //rightup
                    // console.log(y - 1, x + 1);
                    numExtractor(y - 1, x + 1);
                }     
            }

            if ((y < rows.length - 1) && (x > 0)){
                if(numbers.includes((rows[y + 1][x - 1]))){         //leftdown
                    // console.log(y + 1, x - 1);
                    numExtractor(y + 1, x - 1);
                } 
            }

            if((x < rows[y].length - 1) && (y < rows.length - 1)){
                if(numbers.includes((rows[y + 1][x + 1]))){         //rightdown
                    // console.log(y + 1, x + 1);
                    numExtractor(y + 1, x + 1);
                } 
            }
...
function numExtractor(y, x){
    //etract
    let extract = rows[y][x];
    if (!numbers.includes(extract)){
        return;
    }
    ...
    for (let fx = x - 1; fx > -1; fx--){
        if (numbers.includes(rows[y][fx])){
            extract = rows[y][fx] + extract;
            // console.log('<-', rows[y][fx]);
            cutStart = fx;
        } else {
            break;
        }
    }
    for (let fx = x + 1; fx < rows[y].length; fx++){
        if (numbers.includes(rows[y][fx])){
            extract += rows[y][fx];
            // console.log('->', rows[y][fx]);
            cutEnd = fx;
    ...
    debugData.push(extract);
    result += parseInt(extract);

    //remove from row
    const replacement = '.'.repeat(extract.length);
    // console.log('before extract', rows[y]);
    // console.log('cutpoistion:', cutStart, cutEnd);
    let patient = rows[y].split('');
    for (let i = cutStart; i< cutEnd + 1; i++ ){
        patient[i] = '.';
    }
    rows[y] = patient.join('');
    // console.log('after extract', rows[y]);
...
// const testArr = '467..114..'.split('');
// testArr.splice(0, 2 + 1)

// console.log('testas',testArr);

for (let i = 1; i < 10; i++) {
    // console.log(debugData[debugData.length - i]);
    
}

// console.log(debugData.length -1);
console.log(result);
