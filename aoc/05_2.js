...
fs.readFile('.\\aoc\\05_2fin.json', 'utf-8', function (err, data) {
   if (err) throw err;
...
function workwork (input){
    ...
    // const test = new Seed(14, input.soilmap, input.fertmap, input.watmap, input.lightmap, input.tempmap, input.hummap, input.locmap);
    // console.log(test); 
    input.seeds.forEach((seedrange) => {

        const LoopEnd = seedrange[0] + seedrange[1]; 
        const loopStart = seedrange[0];
        console.log(seedrange);
        for (let i = loopStart; i <= LoopEnd; i++) {
            
            
            const newSeed = new Seed(i, input.soilmap, input.fertmap, input.watmap, input.lightmap, input.tempmap, input.hummap, input.locmap); 
            if (i===4002147452){
                continue; 
            }
            if (minLocation > newSeed.loc ){
                minLocation = newSeed.loc;
                console.log('minloc', minLocation, 'Sid', i);
            }
        }
